...
impl BriefcaseSdkReader {
    ...
    pub(crate) async fn prepare_source_target(
        ...
        // Briefcase's documented empty-path delegated list initializes this
        // actor's own app folder. No general folder-create permission is used.
        let initialize = DelegatedListEntries {
            limit: Some(1),
            ..Default::default()
        }
        .prepare()
        ...
        client
            .list_entries_on_behalf_of(
                &app,
                self.proof(&authority, &initialize).await?,
                &initialize,
            )
            .await
            .map_err(map_sdk)?;
